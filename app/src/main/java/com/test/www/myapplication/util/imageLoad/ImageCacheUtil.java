package com.test.www.myapplication.util.imageLoad;
...
import com.test.www.myapplication.util.DiskLruCache;
...
public class ImageCacheUtil implements ImageLoader.ImageCache {
    ...
    public ImageCacheUtil() {
        ...
        try {
            // 获取DiskLruCahce对象
            mDiskLruCache = DiskLruCache.open(getDiskCacheDir(BaseApplication.getContext(), "Rabbit"), getAppVersion(BaseApplication.getContext()), 1, DISKMAXSIZE);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public Bitmap getBitmap(String url) {
        if (mLruCache.get(url) != null) {
            // 从LruCache缓存中取
//            LogUtil.d(BaseActivity.TAG, "从LruCahce获取");
...
//                        LogUtil.d(BaseActivity.TAG, "从DiskLruCahce获取");
                    }
                    return bitmap;
                }
            } catch (IOException e) {
